...
export class HistogramChart implements IHistogramChart {
  ...
  private calculateMargins(options: HistogramChartOptions): {
    ...
    if (showYAxis && this.data.length > 0) {
      ...
      const maxTickLength = Math.max(...ticks.map((tick: number) => {
  ...
  private setupScalesWithBins(bins: HistogramBin[]) {
    const { width = 800, height = 400, margin = { top: 20, right: 20, bottom: 40, left: 60 } } = this.options;

    // X scale for bin positions - use nice scale for better readability
    const xExtent = bins.length > 0 ? d3.extent(bins.flatMap(d => [d.x0, d.x1])) as [number, number] : [0, 1];
  ...
  private addHistogramAxes(
    ...
    const { height = 400, margin = { top: 20, right: 20, bottom: 40, left: 60 }, showXAxis, showYAxis } = this.options;
      ...
      const yAxis = svg
        ...
        .call(
          d3.axisLeft(yScale)
            ...
            .tickSize(5)
            .tickPadding(8)
            .tickFormat((d) => this.options.showDensity 
  ...
  private renderChart() {
    ...
    const containerWidth = containerElement?.parentElement?.clientWidth || this.options.width || 800;
    ...
    const { height = 400, margin = { top: 20, right: 20, bottom: 40, left: 60 }, animate } = this.options;
  ...
  public render(): void {
    ...
    if (typeof window !== 'undefined') {
      this.resizeListener = () => {
        ...
        if (containerElement) {
          const newWidth = containerElement?.parentElement?.clientWidth || this.options.width || 800;

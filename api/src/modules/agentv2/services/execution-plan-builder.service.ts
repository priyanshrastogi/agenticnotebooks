...
export class ExecutionPlanBuilder {
  ...
  async buildPlan(
    ...
    const planningResponse = await this.generatePlanWithLLM(
      query,
      context,
      preferredLLM,
    );
    ...
    const steps: Step[] = planningResponse.steps.map((stepDef) => ({
      ...
      status:
        stepDef.dependsOn.length === 0 ? StepStatus.READY : StepStatus.PENDING,
    }));
    ...
    if (!this.dependencyResolver.validatePlan(plan)) {
      throw new Error(
        'Invalid execution plan - circular dependencies detected',
      );
    }
...
Respond with ONLY the JSON execution plan, no other text.`,
    ...
    try {
      ...
      throw new Error(
        `Invalid execution plan generated by LLM. Please try again.`,
      );
    }
  }
  ...
  private buildContextInfo(context: TaskContext): string {
    ...
    if (context.databases) {
      parts.push(
        `- DATABASE AVAILABLE: ${JSON.stringify(context.databases, null, 2)}`,
      );
    }

    if (context.spreadsheets?.length) {
      parts.push(
        `- SPREADSHEETS AVAILABLE: ${context.spreadsheets.length} files`,
      );
